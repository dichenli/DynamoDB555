...
public class OldInvertedIndex {
	...
	public OldInvertedIndex(String word2, byte[] id2, double tf2,
	...
	public boolean equals(Object other) {
		if(other == null || !(other instanceof OldInvertedIndex)) {
			return false;
		}

		OldInvertedIndex other2 = (OldInvertedIndex) other;
	...
	public static OldInvertedIndex parseInput(String line) {
		if (line == null) {
		...
		return new OldInvertedIndex(word, id, tf, positions, type);
	}
	 ...
	 */
	private static HashMap<String, HashSet<OldInvertedIndex>> items = null;
	 ...
	 * @param item
	 */
	public static void insert(OldInvertedIndex item) {
//		System.out.println("======insert: \n" + item);
		...
		if(items == null) {
			items = new HashMap<String, HashSet<OldInvertedIndex>>();
		}
		HashSet<OldInvertedIndex> set = items.get(item.word);
		if (set == null) {
			set = new HashSet<OldInvertedIndex>();
		}
		set.add(item);
...
//			System.out.println("batchload idfs, items.keyset size: " + items.keySet().size());
			for(IDF idf : idfs) {
				HashSet<OldInvertedIndex> iiset = items.get(idf.word); //iiset: InvertedIndexSet
				for(OldInvertedIndex ii : iiset) {
					ii.idf = idf.idf;
//					System.out.println("====After IDF====\n" + ii);
					batchInsert(ii);
				}
			}
			for(HashSet<OldInvertedIndex> iiset : items.values()) {
				for(OldInvertedIndex ii : iiset) {
					batchInsert(ii);
				}
			}
	...
	private static ArrayList<OldInvertedIndex> readyItems; //all items ready to be sent for batchsave
	private static void batchInsert(OldInvertedIndex item) {
//		System.out.println("======BatchInsert: \n" + item);
		if(readyItems == null) {
			readyItems = new ArrayList<OldInvertedIndex>();
		}
		readyItems.add(item);
		if(readyItems.size() >= 25) {
			HashSet<ByteBuffer> set = new HashSet<ByteBuffer>();
			for(OldInvertedIndex i : readyItems) {
...
//						System.out.println("====After PageRank====\n" + i);
	...
	public static PaginatedQueryList<OldInvertedIndex> query(String word) {
		OldInvertedIndex item = new OldInvertedIndex();
		item.setWord(word);
		DynamoDBQueryExpression<OldInvertedIndex> queryExpression 
		= new DynamoDBQueryExpression<OldInvertedIndex>().withHashKeyValues(item);
		
		PaginatedQueryList<OldInvertedIndex> collection 
		= DynamoTable.mapper.query(OldInvertedIndex.class, queryExpression);
		return collection;
	}
		...
		while(iterator.hasNext()) {
			...
			try {
				while((line = reader.readLine()) != null) {
					OldInvertedIndex item = OldInvertedIndex.parseInput(line);
					if(item != null) {
						OldInvertedIndex.insert(item);
